...
public class CameraFollow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (!m_ballController.IsGrounded && jumpHeightDistance > 0f)
        {
            ...
            m_jumpingCameraOffset.y *= Mathf.Clamp(Mathf.Abs(reducedHeightDistance), 0.0f, Mathf.Abs(reducedHeightDistance) * 10.0f);
            m_jumpingCameraOffset.z *= Mathf.Clamp(Mathf.Abs(reducedHeightDistance), 0.0f, Mathf.Abs(reducedHeightDistance) * 10.0f);
